...
export class MedicineService {
  ...
  private readonly medicinesRefresh$ = new Subject<void>();

  constructor(private http: HttpClient) {}

  /**
   * Emits whenever something external changes medicine stock (e.g. an order placed).
   * Components that show stock should subscribe and re-fetch.
   */
  onMedicinesRefresh(): Observable<void> {
    return this.medicinesRefresh$.asObservable();
  }

  /** Triggers a refresh event for medicine lists (minimal UI-sync mechanism). */
  requestMedicinesRefresh(): void {
    this.medicinesRefresh$.next();
  }
